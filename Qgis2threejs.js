...
//Update this variable with the number of decimal places you want in numbers
var sigFigLength = 2;

// Specify if you want Sig Figs and IND values to be replaced or shown as is in the attribute table of the GIS system.
var replaceSigFigValues = true
var replaceINDValues = true;

// Global Variables for Compass Inset
var container2,
    camera2,
    scene2,
    renderer2,
...
(function () {
    ...
    app.loadProject = function (project) {
        ...
        app.queryMarker = new THREE.Mesh(new THREE.SphereGeometry(opt.r), new THREE.MeshLambertMaterial({
            color: opt.c,
            ambient: opt.c,
            opacity: opt.o,
            transparent: (opt.o < 1)
    ...
    app.buildDefaultCamera = function () {
        ...
        app.camera.position.set(0, -1, 600);
    };
    ...
    app.buildCompass = function () {
        // dom
        app.container2 = document.getElementById('inset');

        // renderer
        app.renderer2 = new THREE.WebGLRenderer({
            alpha: true,
            antialias: true
        });
        app.renderer2.setClearColor(0x000000, 0);
        app.renderer2.setSize(CANVAS_WIDTH, CANVAS_HEIGHT);
        app.container2.appendChild(app.renderer2.domElement);
        // scene
        app.scene2 = new THREE.Scene();

        // camera
        app.camera2 = new THREE.PerspectiveCamera(50, CANVAS_WIDTH / CANVAS_HEIGHT, 1, 1000);
        //app.camera2 = new THREE.OrthographicCamera(CANVAS_WIDTH / -2, CANVAS_WIDTH / 2, CANVAS_HEIGHT /2, CANVAS_HEIGHT / -2, 1, 1000);

        app.camera2.up = app.camera.up; // important!
        app.scene2.add(new THREE.AmbientLight(0x999999));

        // triangle geometry

        var geom = new THREE.Geometry();
        geom.vertices.push(new THREE.Vector3(-20, 40, 0.1));
        geom.vertices.push(new THREE.Vector3(20, 40, 0.1));
        geom.vertices.push(new THREE.Vector3(0, 110, 0.1));
        ...
        var geometry = new THREE.RingGeometry(80, 75, 32);
        var material = new THREE.MeshBasicMaterial({
            color: 0xffffff,
            side: THREE.DoubleSide
        });
        var mesh2 = new THREE.Mesh(geometry, material);
        app.scene2.add(mesh2);

        var geometry = new THREE.PlaneGeometry(200, 3, 1);
        var material = new THREE.MeshBasicMaterial({
            color: 0xffffff,
            side: THREE.DoubleSide
        });
        var plane = new THREE.Mesh(geometry, material);
        app.scene2.add(plane);

        var geometry = new THREE.PlaneGeometry(3, 200, 1);
        var material = new THREE.MeshBasicMaterial({
            color: 0xffffff,
            side: THREE.DoubleSide
        });
        var plane2 = new THREE.Mesh(geometry, material);
        app.scene2.add(plane2);

        //N
        var geometry = new THREE.PlaneGeometry(5, 52, 1);
        var plane3 = new THREE.Mesh(geometry, material);
        plane3.position.set(-20, 150, 0);
        app.scene2.add(plane3);

        var geometry = new THREE.PlaneGeometry(5, 52, 1);
        var plane4 = new THREE.Mesh(geometry, material);
        plane4.position.set(20, 150, 0);
        app.scene2.add(plane4);

        var geometry = new THREE.PlaneGeometry(4, 65, 1);
        var plane5 = new THREE.Mesh(geometry, material);
        plane5.position.set(0, 150, 0);
        plane5.rotation.z = Math.PI / 4.70;
        app.scene2.add(plane5);
    };
    ...
    app.animate = function () {
        ...
        app.camera2.position.sub(app.controls.center); // added by @libe
        app.camera2.position.setLength(CAM_DISTANCE);
    ...
    app.showInfo = function () {
        ...
        $("#popupIcon").css("opacity", "0.6");
        app.popup.show();
    };
    app.showQueryResult = function (point, layerId, featureId) {
        $("#popupIcon").css("opacity", "0.9");
        console.log(this.point);
        console.log(app.project.layers[layerId]);
        var layer, r = [];
        ...
        // clicked coordinates
        var demLayer = app.project.layers[0];
        var z = demLayer.getZ(point.x, point.y);
        var pt = app.project.toMapCoordinates(point.x, point.y, z);
        ...
        if (typeof proj4 === "undefined") r.push([pt.x.toFixed(2), pt.y.toFixed(2), pt.z.toFixed(2)].join(", "));
            ...
            r.push(Q3D.Utils.convertToDMS(lonLat[1], lonLat[0]) + ", Depth " + Math.round(pt.z.toFixed(2)) + " (m)");
        }
        r.push("</td></tr></table>");
        if (layerId !== undefined && featureId !== undefined && layer.a !== undefined) {
            ...
            for (var i = 0, l = layer.a.length; i < l; i++) {
                // FLAG START OF STRING REPLACEMENT FOR IND DATA AND Qt DATA/TIME
                var foo = f.a[i];
                var bar = false;
                if (typeof foo == 'string') {
                    var bar = foo.startsWith("PyQt4.QtCore.QDate(");
                }
                if (bar) {
                    foo = foo.replace("PyQt4.QtCore.QDate(", "");
                    foo = foo.replace(")", "");
                    var array = JSON.parse("[" + foo + "]");
                    foo = array[1] + "/" + array[2] + "/" + array[0];
                    console.log(foo);
                } else if (foo == -9 && replaceINDValues) {
                    foo = foo.toString();
                    foo = foo.replace("-9", "IND");
                }
                if (typeof foo == 'number' && replaceSigFigValues && !Number.isInteger(foo)) {
                    foo = foo.toFixed(sigFigLength);
                }
                r.push("<tr><td>" + layer.a[i] + "</td><td>" + foo + "</td></tr>");
            }
            r.push("</table>");
        }
    ...
    app.canvasClicked = function (e) {
        ...
        for (var i = 0, l = objs.length; i < l; i++) {
            ...
            var object = obj.object,
                layerId, featureId;
            while (object) {
                layerId = object.userData.layerId, featureId = object.userData.featureId;
                if (layerId !== undefined) break;
                object = object.parent;
            }
            // highlight clicked object
            app.highlightFeature((layerId === undefined) ? null : layerId, (featureId === undefined) ? null : featureId);
            app.showQueryResult(obj.point, layerId, featureId);
            console.log("You have clicked " + obj.point, + " " + layerId, + " " + featureId);
...
Q3D.DEMLayer.prototype.buildFrame = function (block, color, z0) {
    ...
    var geom = new THREE.Geometry();
    geom.vertices.push(new THREE.Vector3(-hw, -hh, z0), new THREE.Vector3(hw, -hh, z0), new THREE.Vector3(hw, hh, z0), new THREE.Vector3(-hw, hh, z0), new THREE.Vector3(-hw, -hh, z0));
    ...
    pts.forEach(function (pt) {
        var geom = new THREE.Geometry();
        geom.vertices.push(new THREE.Vector3(pt[0], pt[1], pt[2]), new THREE.Vector3(pt[0], pt[1], z0));
        var obj = new THREE.Line(geom, mat);
        this.addObject(obj, false);
        dem.aObjs.push(obj);
    }, this);
...
Q3D.Utils.convertToDMS = function (lat, lon) {
    ...
    return ((lat < 0) ? "S" : "N") + toDMS(Math.abs(lat)) + ", " + ((lon < 0) ? "W" : "E") + toDMS(Math.abs(lon));
};
